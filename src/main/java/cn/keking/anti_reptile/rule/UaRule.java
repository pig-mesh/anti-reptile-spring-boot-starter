...
public class UaRule extends AbstractRule {
	...
	protected boolean doExecute(HttpServletRequest request, HttpServletResponse response) {
		...
		if (DeviceType.UNKNOWN.equals(deviceType)) {
			System.out.println("Intercepted request, uri: " + request.getRequestURI() + " Unknown device, User-Agent: "
					+ userAgent.toString());
			return true;
		}
		else if (OperatingSystem.UNKNOWN.equals(os) || OperatingSystem.UNKNOWN_MOBILE.equals(os)
		...
		if (!uaRule.isAllowedIot() && (DeviceType.DMR.equals(deviceType) || DeviceType.GAME_CONSOLE.equals(deviceType)
